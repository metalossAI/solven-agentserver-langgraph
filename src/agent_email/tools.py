...
def get_composio_gmail_tools(user_id, thread_id):
    ...
    tools = composio.tools.get(
        user_id=user_id,
        toolkits=["GMAIL"],
        modifiers=[before_execute_modifier_gmail_fetch, save_attachment_modifier],
    )
    return tools
...
def get_composio_outlook_tools(user_id, thread_id):
    ...
    tools = composio.tools.get(
        user_id=user_id,
        toolkits=["OUTLOOK"],
        modifiers=[before_execute_modifier_outlook_fetch, save_attachment_modifier],
    )
    return tools
...
def create_save_attachment_modifier(user_id: str, thread_id: str):
    ...
    @after_execute(
        tools=["GMAIL_GET_ATTACHMENT", "OUTLOOK_DOWNLOAD_OUTLOOK_ATTACHMENT"]
    )
...
def before_execute_modifier_gmail_fetch(
    ...
    params["arguments"]["max_results"] = 30
...
def process_attachment(
    ...
    try:
        ...
        print(
            f"[after_execute_modifier_save_attachment] Converting to markdown with Docling"
        )
        ...
        print(
            f"[after_execute_modifier_save_attachment] Saving markdown to S3: {markdown_filename}"
        )
