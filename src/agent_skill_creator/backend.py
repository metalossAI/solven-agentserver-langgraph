...
class SandboxBackend(SandboxBackendProtocol):
	...
	def __init__(self, runtime: ToolRuntime[AppContext]):
		...
		if runtime.context.user is None:
			raise RuntimeError("Cannot initialize SandboxBackend: runtime.context.user is None")
		...
		# Get skill_name from context.skill_create.name if available (for skill creation)
		self._skill_name = None
		if hasattr(runtime.context, 'skill_create') and runtime.context.skill_create:
	...
	async def _upload_mount_scripts(self) -> None:
		...
		# Go up from agent_skill_creator to src, then to e2b_sandbox/scripts
	...
	async def _initialize_workspace(self) -> None:
		...
		if not pyproject_exists:
			...
			# Go up from agent_skill_creator to src, then to e2b_sandbox/resources
			resources_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "e2b_sandbox", "resources")
	...
	async def _run_isolated(self, command: str, timeout: int = 30000):
		...
		For all workspaces:
		- / binds to /mnt/r2/threads/{thread_id} (workspace root)
		- /.solven/skills/system binds to /mnt/r2/skills/system (read-only) - for reading system skills
		
		Only the workspace and user skills are bound in bwrap. User skills are mounted via rclone
		at /mnt/r2/skills/ and bound into the bwrap environment at /.solven/skills/.
		"""
		import shlex
		await self._ensure_initialized()
		
		# Verify user skills mount exists before binding
		user_skills_mount = f"/mnt/r2/skills/{self._user_id}"
		user_skills_exists = await self._sandbox.files.exists(user_skills_mount)
		if not user_skills_exists:
			raise RuntimeError(f"User skills mount not found at {user_skills_mount}. Ensure R2 buckets are mounted.")
		
		# Build bwrap command
		bwrap_cmd = [
			"bwrap",
			
			# Mount R2 workspace as / (writable) - this is threads/thread_id
			...
			# User skills (writable) - allows agent to read and modify user skills
			# Maps: /mnt/r2/skills/{user_id} -> /.solven/skills/
			"--bind", user_skills_mount, "/.solven/skills",
		]
...
"""
	...
	async def get_skill_content(self, skill_name: str) -> Optional[str]:
		"""
		Get the SKILL.md content directly from the workspace root.
		
		For skill creation workspaces, SKILL.md is copied from skill-creator
		to the workspace root during initialization.
		
		Args:
			skill_name: Name of the skill (unused, kept for compatibility)
			
		Returns:
			SKILL.md content as string if it exists, None otherwise
		"""
		await self._ensure_initialized()
		
		# Read SKILL.md directly from workspace root (copied from skill-creator)
		skill_md_path = f"{self._r2_workspace}/SKILL.md"
