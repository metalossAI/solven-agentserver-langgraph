...
class S3Backend(BackendProtocol):
    ...
    Works with Supabase Storage S3, MinIO, AWS S3, or any S3-compatible storage.
    ...
    Args:
        ...
        endpoint_url: S3 endpoint URL
            * For Supabase: https://project_ref.storage.supabase.co/storage/v1/s3
            * For MinIO: http://localhost:9000
            * For AWS S3: leave empty (uses default AWS endpoints)
        access_key: S3 access key (from Supabase project settings or S3 provider)
        secret_key: S3 secret key (from Supabase project settings or S3 provider)
        region: AWS region (default: "us-east-1", use project region for Supabase)
    ...
    Note: Supabase Storage S3 requires path-style URLs, which is automatically
    enabled when the endpoint contains 'supabase.co'.
    """
    ...
    def s3_client(self):
        ...
        if self._s3_client is None:
            ...
            # Supabase Storage S3 requires path-style addressing
            config = None
            if self._s3_endpoint and 'supabase.co' in self._s3_endpoint:
                # Use path-style addressing for Supabase (required)
...
def get_s3_backend_from_env() -> S3Backend:
    ...
    - S3_ENDPOINT_URL: S3 endpoint URL
        ...
        * For MinIO: http://localhost:9000
        * For AWS S3: leave empty (uses default AWS endpoints)
    - S3_ACCESS_KEY_ID: Access key (from Supabase project settings or S3 provider)
    - S3_SECRET_KEY: Secret key (from Supabase project settings or S3 provider)
    - S3_PREFIX: Optional prefix for all paths (default: "")
    - S3_REGION: AWS region (default: "us-east-1", use project region for Supabase)
...
class S3FileUploader:
    """
    Async S3 file uploader utility for handling file uploads from Composio and other sources.
    Provides methods for single file uploads, batch uploads, and file conversion to markdown.
    
    Works with Supabase Storage S3, MinIO, AWS S3, or any S3-compatible storage.
    Automatically enables path-style URLs for Supabase endpoints.
    """
    ...
    async def _get_s3_client(self):
        """Get or create async S3 client (non-blocking)"""
        if self._s3_client is None:
            # Check if endpoint is Supabase (requires path-style URLs)
            # Supabase Storage S3 requires path-style addressing
            config = None
            ...
            self._s3_client = await asyncio.to_thread(create_client)
        return self._s3_client
    ...
    async def _ensure_bucket_exists(self):
        ...
        try:
            ...
            try:
                await asyncio.to_thread(client.create_bucket, Bucket=self.bucket)
                self._bucket_checked = True
            except ClientError as e:
                print(f"Warning: Could not create bucket {self.bucket}: {e}", flush=True)
                self._bucket_checked = True
    ...
    async def upload_file(
        ...
        """
        Upload a single file to S3 (async, non-blocking).
        ...
        try:
            ...
            print(f"[S3FileUploader] Error uploading {local_path} to {s3_key}: {e}", flush=True)
            return False
    ...
    async def upload_file_content(
        ...
        """
        Upload file content (bytes) directly to S3 (async, non-blocking).
        ...
        try:
            ...
            print(f"[S3FileUploader] Error uploading content to {s3_key}: {e}", flush=True)
            return False
    ...
    async def upload_file_text(
        ...
        """
        Upload text content to S3 (async, non-blocking).
    ...
    async def batch_upload_files(
        ...
        """
        Upload multiple files to S3 in batch (async, non-blocking).
        
        Args:
            files: List of tuples (local_path, relative_s3_path)
            base_s3_prefix: Base prefix for all S3 keys
            metadata: Optional metadata dict to apply to all files
        
        Returns:
            Dict mapping s3_key -> success (bool)
        """
        results = {}
        ...
        for local_path, relative_path in files:
            s3_key = f"{base_s3_prefix}/{relative_path}".lstrip('/')
            task = self.upload_file(local_path, s3_key, metadata=metadata)
            tasks.append((s3_key, task))
        
        # Wait for all uploads to complete
        for s3_key, task in tasks:
            results[s3_key] = await task
    ...
    async def upload_and_convert_to_markdown(
        ...
        """
        Upload a file and convert it to markdown, uploading both versions.
        
        Args:
            local_path: Path to local file
            s3_key_md: S3 key for markdown version
            s3_key_original: Optional S3 key for original file (if None, only markdown is uploaded)
            metadata: Optional metadata dict
        
        Returns:
            Tuple of (markdown_upload_success, original_upload_success)
        """
        from docling.document_converter import DocumentConverter
        
        try:
            # Convert to markdown (this is CPU-bound, run in thread pool)
            converter = DocumentConverter()
            conversion_result = await asyncio.to_thread(converter.convert, local_path)
            markdown_content = conversion_result.document.export_to_markdown()
            
            # Upload markdown version
            md_success = await self.upload_file_text(
                markdown_content,
                s3_key_md,
            ...
            if s3_key_original:
                original_success = await self.upload_file(
                    local_path,
                    s3_key_original,
                    metadata=metadata
                )
            
            return (md_success, original_success)
        except Exception as e:
            print(f"[S3FileUploader] Error converting and uploading {local_path}: {e}", flush=True)
            return (False, False)


async def get_s3_file_uploader() -> S3FileUploader:
    """
    Create S3FileUploader instance from environment variables.
