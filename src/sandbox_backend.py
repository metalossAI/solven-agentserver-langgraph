...
class SandboxBackend(SandboxBackendProtocol):
	...
	def __init__(self, runtime_context: AppContext):
		...
		self._init_lock = asyncio.Lock()  # Lock for async initialization
	...
	async def _setup_symlinks(self) -> None:
		...
		if await asyncio.to_thread(self._sandbox.files.exists, system_skills_path):
			await self._run_command(
		...
		if self._r2_ticket_path:
			if not await asyncio.to_thread(self._sandbox.files.exists, self._r2_ticket_path):
	...
	async def _setup_python_environment(self) -> None:
		"""Set up Python environment with uv and verify it works.
		Uses local cache directory to avoid slow R2 network filesystem.
		"""
		venv_path = f"{self._base_path}/.venv"
		pyproject_path = f"{self._base_path}/pyproject.toml"
		
		venv_exists = await asyncio.to_thread(self._sandbox.files.exists, venv_path)
		pyproject_exists = await asyncio.to_thread(self._sandbox.files.exists, pyproject_path)
		if venv_exists and pyproject_exists:
			print(f"[Workspace] ✓ Python environment already exists", flush=True)
			await self._verify_python_environment()
			return
		
		print(f"[Workspace] Initializing Python environment...", flush=True)
		
		# Initialize uv project with local cache
		await self._run_command(
			f"cd {self._base_path} && UV_CACHE_DIR=/tmp/uv-cache uv init --python 3.12 --no-readme",
			timeout=15000,
			description="Initializing uv project"
		)
		
		# Create venv if it doesn't exist, with local cache
		if not await asyncio.to_thread(self._sandbox.files.exists, venv_path):
			print(f"[Workspace] Creating Python virtual environment...", flush=True)
			await self._run_command(
				f"cd {self._base_path} && UV_CACHE_DIR=/tmp/uv-cache uv venv",
				timeout=15000,
		...
		print(f"[Workspace] ✓ Python environment ready", flush=True)
	...
	async def _verify_python_environment(self) -> None:
		...
		try:
			...
			result = await asyncio.to_thread(
				...
				f"cd {self._base_path} && .venv/bin/python --version",
				timeout=5000
			)
	...
	async def _setup_node_environment(self) -> None:
		"""Set up Node.js environment with Bun (fast initialization only).
		Uses local cache directory to avoid slow R2 network filesystem.
		"""
		package_json_path = f"{self._base_path}/package.json"
		
		if await asyncio.to_thread(self._sandbox.files.exists, package_json_path):
			print(f"[Workspace] ✓ Bun project already exists", flush=True)
			return
		
		print(f"[Workspace] Initializing Node.js environment (fast mode)...", flush=True)
		
		# Just initialize Bun project - no packages yet (fast!)
		# Packages will be installed on-demand when needed
		# Use local cache directory for speed
		await self._run_command(
			f"cd {self._base_path} && BUN_INSTALL_CACHE_DIR=/tmp/bun-cache bun init -y",
			timeout=10000,
			description="Initializing Bun project"
		)
		
		print(f"[Workspace] ✓ Node environment initialized (packages install on-demand)", flush=True)
...
"""
	...
	async def _setup_python_venv(self) -> None:
		"""
		Create Python virtual environment in workspace using uv.
		This runs OUTSIDE bwrap during initial setup.
		Uses local cache directory to avoid slow R2 network filesystem.
		"""
		venv_path = f"{self._base_path}/.venv"
		
		# Check if venv already exists (non-blocking)
		exists = await asyncio.to_thread(self._sandbox.files.exists, f"{venv_path}/bin/python")
		if exists:
			print(f"[Python] ✓ Virtual environment already exists", flush=True)
			return
	
		print(f"[Python] Creating virtual environment...", flush=True)
		result = await asyncio.to_thread(
			self._sandbox.commands.run,
			f"cd {self._base_path} && UV_CACHE_DIR=/tmp/uv-cache uv venv", 
			timeout=15000
		...
		if result.exit_code != 0:
			print(f"[Python] ⚠️  Failed to create venv: {result.stderr}", flush=True)
			return
		
		# Verify it was created (non-blocking)
		exists = await asyncio.to_thread(self._sandbox.files.exists, f"{venv_path}/bin/python")
		if exists:
			print(f"[Python] ✓ Virtual environment created at {venv_path}", flush=True)
		else:
			print(f"[Python] ⚠️  venv creation succeeded but python not found", flush=True)
	
	async def _setup_node_env(self) -> None:
		"""
		Initialize Node/Bun project in workspace.
		This runs OUTSIDE bwrap during initial setup.
		Uses local cache directory to avoid slow R2 network filesystem.
		"""
		pkg_json_path = f"{self._base_path}/package.json"
		
		# Check if package.json already exists (non-blocking)
		exists = await asyncio.to_thread(self._sandbox.files.exists, pkg_json_path)
		if exists:
			print(f"[Bun] ✓ package.json already exists", flush=True)
			return
		
		# Initialize with bun, using local cache directory (not R2-backed)
		# This significantly speeds up package operations
		print(f"[Bun] Initializing project...", flush=True)
		result = await asyncio.to_thread(
			self._sandbox.commands.run,
			f"cd {self._base_path} && BUN_INSTALL_CACHE_DIR=/tmp/bun-cache bun init -y", 
			timeout=15000
		...
		if result.exit_code != 0:
			print(f"[Bun] ⚠️  Failed to initialize: {result.stderr}", flush=True)
			return
		
		# Verify it was created (non-blocking)
		exists = await asyncio.to_thread(self._sandbox.files.exists, pkg_json_path)
		if exists:
			print(f"[Bun] ✓ Project initialized at {pkg_json_path}", flush=True)
		else:
			print(f"[Bun] ⚠️  bun init succeeded but package.json not found", flush=True)
	...
	async def _setup_rclone_mounts(self) -> None:
		...
		# Upload scripts to sandbox (files.write is synchronous in E2B)
		try:
		...
		try:
			result = await asyncio.wait_for(
				self._sandbox.commands.run(mount_cmd, timeout=30000),
				timeout=35  # Python-level timeout (35 seconds)
			)
			...
			print(f"[rclone] ✗ Thread mount timed out after 35 seconds", flush=True)
			# Try to get log
			try:
				log_result = await self._sandbox.commands.run("tail -50 /tmp/rclone-thread.log 2>&1 || echo 'No log file'", timeout=5000)
				print(f"[rclone] rclone log tail: {log_result.stdout}", flush=True)
		...
		await asyncio.sleep(3)  # Give FUSE mounts time to initialize
...
"""
	...
	async def _ensure_workspace_configured(self) -> None:
		...
		marker_exists = await asyncio.to_thread(self._sandbox.files.exists, config_marker_path)
		if marker_exists:
			# Verify that critical directories actually exist (prevent inconsistent state)
			print(f"[Workspace] Marker found, verifying workspace structure...", flush=True)
			verify_cmd = f"test -d {self._base_path}/.solven/skills && echo 'OK' || echo 'MISSING'"
			verify_result = await self._sandbox.commands.run(verify_cmd, timeout=5000)
			
			if "OK" in verify_result.stdout:
				print(f"[Workspace] ✓ Configured and verified", flush=True)
				return
			else:
				print(f"[Workspace] ⚠ Marker exists but directories missing - reconfiguring", flush=True)
				# Remove stale marker and continue with setup
				await self._sandbox.commands.run(f"rm -f {config_marker_path}", timeout=5000)
		
		print(f"[Workspace] Configuring with bwrap isolation...", flush=True)
		...
		try:
			...
			# Setup Python environment (creates .venv in workspace) (non-blocking)
			print(f"[Workspace] Setting up Python environment...", flush=True)
			await self._setup_python_venv()
			
			# Setup Node environment (creates package.json in workspace) (non-blocking)
			print(f"[Workspace] Setting up Node/Bun environment...", flush=True)
			await self._setup_node_env()
			
			# Create marker (non-blocking)
			await asyncio.to_thread(self._sandbox.files.write, config_marker_path, f"Configured at {datetime.now().isoformat()}")
			
			print(f"[Workspace] ✓ Ready - workspace configured with Python, Node, and isolated as /", flush=True)
	...
	async def _filter_unwanted_commands(self, command: str) -> str:
		...
		for unwanted_command in unwanted_commands.keys():
			if command.startswith(unwanted_command):
	...
	async def _run_bwrap_direct(self, bash_command: str, timeout: int = 10000):
		...
		# Wrap command with venv activation if available
		wrapped_command = f"""
# Activate Python venv if exists (isolated to this workspace)
if [ -f /.venv/bin/activate ]; then
    source /.venv/bin/activate
fi
...
"""
		...
		bwrap_cmd = [
			...
			# All workspace files, .venv, node_modules are here
		...
		bwrap_cmd.extend([
			...
			# These ensure Node/Bun use ONLY workspace environment
			...
			"--setenv", "UV_NO_SYNC", "1",  # Don't sync with system
			...
			# === ENVIRONMENT: PATH (Workspace First!) ===
			# Critical: Workspace binaries take precedence over system
		...
		print(f"[bwrap] Isolated workspace as / (Python: /.venv, Node: /node_modules)", flush=True)
	...
	async def ensure_python_init(self) -> bool:
		"""
		Ensure Python environment exists (for runtime use inside bwrap).
		
		Note: During initial setup, use _setup_python_venv() instead.
		This method is for checking/creating venv from within bwrap context.
		
		Returns True if venv exists or was created successfully.
		"""
		try:
			# Check if venv exists (in workspace root, appears as /.venv inside bwrap)
			venv_check = await self._run_isolated("[ -f /.venv/bin/python ] && echo 'exists' || echo 'missing'", timeout=5000)
			
			if 'exists' in venv_check.stdout:
				print("[Python] ✓ Virtual environment exists at /.venv", flush=True)
				return True
			
			# Create venv with uv (in workspace root)
			print("[Python] Creating virtual environment at /.venv...", flush=True)
			result = await self._run_isolated("uv venv /.venv", timeout=15000)
			
			if result.exit_code != 0:
				print(f"[Python] ⚠️  Failed to create venv: {result.stderr}", flush=True)
				return False
			
			print("[Python] ✓ Virtual environment created", flush=True)
			return True
	...
	async def ensure_node_init(self) -> bool:
		"""
		Ensure Node/Bun environment exists (for runtime use inside bwrap).
		
		Note: During initial setup, use _setup_node_env() instead.
		This method is for checking/creating package.json from within bwrap context.
		
		Returns True if package.json exists or was created successfully.
		"""
		try:
			# Check if package.json exists (in workspace root, appears as / inside bwrap)
			pkg_check = await self._run_isolated("[ -f /package.json ] && echo 'exists' || echo 'missing'", timeout=5000)
			
			if 'exists' in pkg_check.stdout:
				print("[Bun] ✓ package.json exists", flush=True)
				return True
			
			# Initialize with bun (in workspace root)
			print("[Bun] Initializing project at /...", flush=True)
			result = await self._run_isolated("cd / && bun init -y", timeout=15000)
			
			if result.exit_code != 0:
				print(f"[Bun] ⚠️  Failed to initialize: {result.stderr}", flush=True)
				return False
			
			print("[Bun] ✓ Project initialized", flush=True)
			return True
...
"""
	...
	async def agrep_raw(self, pattern: str, path: Optional[str] = None, glob: Optional[str] = None) -> list[GrepMatch] | str:
		...
		try:
			...
			# Build grep command based on parameters
			if glob:
				# Handle glob pattern correctly
				if '**/' in glob:
					# Extract the actual file pattern after **/
					file_pattern = glob.split('**/')[-1]
					# Use find with -name to search recursively
					grep_cmd = f"find {shlex.quote(abs_path)} -type f -name {shlex.quote(file_pattern)} -exec grep -nH {shlex.quote(pattern)} {{}} + 2>/dev/null || true"
				elif '/' in glob:
					# Pattern has directory component (e.g., "dir/*.txt")
					pattern_parts = glob.rsplit('/', 1)
					search_dir = f"{abs_path}/{pattern_parts[0]}" if abs_path != '/' else f"/{pattern_parts[0]}"
					file_pattern = pattern_parts[1]
					grep_cmd = f"find {shlex.quote(search_dir)} -maxdepth 1 -type f -name {shlex.quote(file_pattern)} -exec grep -nH {shlex.quote(pattern)} {{}} + 2>/dev/null || true"
				else:
					# Simple pattern without directory (e.g., "*.docx")
					grep_cmd = f"find {shlex.quote(abs_path)} -maxdepth 1 -type f -name {shlex.quote(glob)} -exec grep -nH {shlex.quote(pattern)} {{}} + 2>/dev/null || true"
			else:
				# Search in path or entire workspace
				grep_cmd = f"grep -rn {shlex.quote(pattern)} {shlex.quote(abs_path)} 2>/dev/null || true"
			
			result = await self._run_isolated(grep_cmd, timeout=30000)
