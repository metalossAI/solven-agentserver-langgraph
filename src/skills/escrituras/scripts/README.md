...
| `example_fill.py` | Reference implementation: fill a DOCX by replacing `{{KEY}}` with values from JSON. Use as base for model-specific fill scripts. |
| **fill/** | Model-specific fill scripts go here (e.g. `fill/42_modelo_fill.py`). Create `{model_anchor}_fill.py` in this folder. |
